...
const getUserDoc = async (
	...
	if (!useCache) {
		console.log(`useCache [${useCache}]... getting user doc from firebase DB`);
		const { exists, data } = await getUserDocFromFirestore(userId);
		if (exists) {
			saveDocInCache(userId, stringify(data), cacheDurationInMinutes);
		}
		return { exists, data };
	}
	...
	if (useCache) {
		...
		const isCacheExpired =
			expiration !== null && new Date().getTime() > expiration;
		...
		console.log(`useCache [${useCache}]... getting user doc from cache`);
		return { exists, data };
	}
};
