...
function DailyTradesModal(props) {
	const {
		activeDateDate,
		activeMonth,
		activeYear,
							...
							<div className="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left">
								...
								<div className="mt-2">
									<div className="w-full flex flex-wrap">
										...
										{!!activeDateTradeLogs?.length &&
											activeDateTradeLogs.map((activeTradeLog) => {
												const profit = getTotalProfitFromTrades([
													{ ...activeTradeLog },
												]);
														...
														<div className="h-full p-6 rounded-lg border-2 border-gray-300 flex flex-col">
															<div className="flex flex-wrap justify-between">
																<div className="flex flex-col">
																	<h2 className="text-sm tracking-widest title-font mb-1 font-medium text-left">
																		{activeTradeLog.ticker}
																	</h2>
																	<h1 className="text-2xl title-font mb-1 font-medium">
																		{getOrganizationFromTicker(
																			activeTradeLog?.ticker
																		) || 'Anonymous'}
																	</h1>
																</div>
															...
															<div className="rounded bg-white border border-gray-200 my-3 py-2">
																<div className="flex flex-wrap content-start text-center">
																	<div className="p-2 sm:w-1/3 lg:w-1/4 w-1/2">
																		<h2 className="title-font text-sm text-gray-900">
																			{activeTradeLog.openingPrice}
																		</h2>
																		<p className="leading-relaxed text-xs text-gray-400 font-light">
																			Open
																		</p>
																	</div>
																	<div className="p-2 sm:w-1/3 lg:w-1/4 w-1/2">
																		<h2 className="title-font text-sm text-gray-900">
																			{activeTradeLog.closingPrice}
																		</h2>
																		<p className="leading-relaxed text-xs text-gray-400 font-light">
																			Close
																		</p>
															...
															<div className="log-actions flex justify-center sm:justify-end">
																<button
																	className="lg:mt-2 xl:mt-0 btn-small text-white inline bg-yellow-500 border-0 text-xs py-2 px-4 focus:outline-none hover:bg-yellow-600 rounded"
																	onClick={() => onEditTradeLog(activeTradeLog)}
																>
																	Edit
																</button>
																<button
																	className="lg:mt-2 xl:mt-0 ml-1 text-white bg-red-500 border-0 text-xs py-2 px-4 focus:outline-none hover:bg-red-600 rounded"
																	onClick={() =>
																		onDeleteTradeLog(activeTradeLog)
																	}
																>
																	Delete
																</button>
