...
class AddNewTradeFormModal extends Component {
  ...
  scrollToForm() {
    document.querySelector('#AddNewTradeForm').scrollIntoView({ behavior: 'smooth' });
  }
  ...
  async onCreateTrade(e) {
    ...
    const { error: validateTradeLogError, isValid: isTradeLogValid } = this.validateFields();
  ...
  onCreateAndAddMore(e) {
    ...
    const { error: validateTradeError, isValid: isTradeValid } = this.validateFields();
